...
async function googleVision(image) {
 const auth=await fetch('http://metadata.google.internal/computeMetadata/v1/instance/service-accounts/default/token',{headers:{'Metadata-Flavor':'Google'},signal:AbortSignal.timeout(5000)});
 ...
 return createServer(async(req,res)=>{
  ...
  try {
   ...
   if(path==='/api/ocr' && req.method==='POST'){
    ...
    if(++count>60) throw fail(429,'使用頻率過高，請一分鐘後再試');
  ...
  }catch(error){if(!res.headersSent) json(res,error.status || 503,{error:error.status?error.message:'雲端辨識暫時無法使用，請稍後再試'});}
 });
}
